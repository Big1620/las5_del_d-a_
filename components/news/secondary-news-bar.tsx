...
export function SecondaryNewsBar({ articles }: SecondaryNewsBarProps) {
  ...
  return (
    <div className="py-0">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6 lg:gap-x-8 divide-y sm:divide-y-0 sm:divide-x divide-[#E5E5E5] dark:divide-gray-600">
        {articles.slice(0, 4).map((article) => {
          const category = article.categories?.[0];
          const url = getArticleUrl(article.slug);
          return (
            <article key={article.id} className="group py-4 sm:py-0 sm:px-5 first:sm:pl-0 last:sm:pr-0">
              {category && (
                <div className="mb-2">
